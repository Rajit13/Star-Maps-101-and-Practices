...
\textit{This book is written based on my experience as both a participant in two international Olympiads and as a team leader for IOAA since 2018 (9 years now!). I have also incorporated my experience as a stargazer and amateur astronomer into it.}\\

The book has dedicated \textbf{web-page now}: Go explore! -- \url{https://rajit13.github.io/starcharts101/}

\vfill{\LaTeX  \;  compiled and  \copyright \; 2026 by \textsf{Fahim Rajit Hossain Shwadhin} is licensed under CC BY-NC-SA 4.0. To view a copy of this license, visit \url{http://creativecommons.org/licenses/by-nc-sa/4.0/} }\\
...
Most of the brighter stars were assigned their first systematic names by the German astronomer Johann Bayer in 1603, in his star atlas \textit{Uranometria}. Bayer assigned a lowercase Greek letter, such as alpha ($\alpha$), beta ($\beta$), gamma ($\gamma$), etc., to each star he catalogued, combined with the Latin name of the star’s parent constellation in genitive (possessive) form. (See all 88 modern constellations for the genitive forms later on the table \ref{cons_table1}.) For example, Aldebaran is designated $\alpha$ Tauri (pronounced \textit{Alpha Tauri}), which means ``Alpha of the constellation Taurus".\\
...
\prt{$\alpha$ And }               & \prt{$\beta$ Aur}  & $\alpha$ CMa  & $\alpha$ Gem & $\alpha$ Ori & $\alpha$ Psc      & $\beta$ UMa   \\
$\beta$ And                 & $\alpha$ Boo & $\beta$ CMa   & $\beta$ Gem  & $\beta$ Ori  & $\alpha$ Sco      & $\alpha$ UMi \\
\prt{$\alpha$ Ari}                & $\beta$ Boo  & $\alpha$ CMi  & $\alpha$ Leo & $\alpha$ Oph & $\beta$ Sco       & $\beta$ UMi  \\
$\beta$ Ari                 & $\alpha$ Cap & \prt{$\beta$ CMi}   & $\beta$ Leo  & $\beta$ Oph  & $\alpha$ Ser      & $\beta$ Cyg  \\
...
$\alpha$ Aur & \prt{$\alpha$ Cet} & $\alpha$ Cyg  & $\beta$ Lyr  & $\beta$ Peg  & $\alpha$ UMa      &             
\end{tabular}
...
%https://watcheroftheskies.net/stars/star-maps.html 
\begin{figure}[H]
    \centering
...
\textbf{Draw and Solve the scenario here}
		\vspace{13cm}
\end{minipage}}
\clearpage
...
\fbox{\begin{minipage}{43em}
For data related to the problems below refer to the Messier Object List table \ref{Messier_list}. 
\end{minipage}}\\
...
\subsection*{Exmaple Problem: The Sagittarius Dwarf Galaxy}


{ \footnotesize
	In 1994, Rodrigo Ibata, Mike Irwin, and Gerry Gilmore discovered a dwarf spheroidal galaxy near the center of the Milky Way. Because it lies in the direction of the constellation Sagittarius, it was named the \emph{Sagittarius Dwarf Galaxy}. In this problem, our goal is to determine the center of this galaxy.\\
	
	\begin{wrapfigure}{r}{0.4\linewidth}
		\centering
		\includegraphics[width=\linewidth]{stereo_geo0.pdf}
		\caption{Stereographic Projection}
		\label{sgr_drawf_stereo}
	\end{wrapfigure}
	
	To do this, we need to plot the galaxy on a sky map. We have a sky chart in which the celestial sphere has been projected onto a plane using a stereographic projection. This projection preserves angles, but not areas. Under this projection, a point on the sphere is mapped to a point on a circle, as shown in the figure \ref{sgr_drawf_stereo}.\\
	
	The stereographic projection relation\footnote{Geometric derivation of the relation can be found later in the book here: \ref{stereo_projection}} is
	
	\[
	r(\theta)=2R\tan\!\left(\frac{\theta}{2}\right),
	\]
	
	where \(R\) is the radius of the celestial sphere.\\
	
	The map provided to you (Fig \ref{sgr_dwarf_map}) is a stereographic chart centered on the star Arcturus (\(\alpha\) Boo).
	
	\begin{enumerate}[a.]
		\itemsep0em
		\item As a first step, identify the galaxy. The Sagittarius Dwarf Galaxy has an angular thickness of approximately \(12^\circ\). First calculate the chart radius \(R\), then draw the Milky Way and the galaxy on the map.
		
		\item
		The edge of the dwarf galaxy is not clearly defined. However, using data from the \emph{Gaia} satellite, it has been found that three stars lie on the apparent boundary of the galaxy. These stars are \(\pi\), \(\gamma\), and \(h1\) of the constellation Sagittarius.
		
		Using the data for these stars, draw the galaxy on the chart.
		
		\item
		All galaxies contain a central star cluster. Such clusters have lower surface brightness than the rest of the galaxy and are therefore much easier to detect than the surrounding galaxy itself.
		
		Using the geometric shape of the galaxy, determine the true center of the Sagittarius Dwarf Galaxy and mark it with a ``+'' sign.
		
		\item
		The center of this galaxy contains a globular cluster from the Messier catalog. Which Messier object corresponds to the central cluster of the Sagittarius Dwarf Galaxy?
	\end{enumerate}}
	
	\begin{table}[H]
		\centering
		\caption{Coordinates of the stars used in the construction.}
		\begin{tabular}{lcc}
			\hline
			\textbf{Object} & \textbf{RA} & \textbf{Dec} \\
			\hline
			Arcturus (\(\alpha\) Boo) & \(14^{\rm h}16^{\rm m}\) & \(+19^\circ03'\) \\
			North Galactic Pole & \(12^{\rm h}53^{\rm m}\) & \(+27^\circ06'\) \\
			Alnasl (\(\gamma\) Sgr) & \(18^{\rm h}07^{\rm m}\) & \(-30^\circ25'\) \\
			\(\pi\) Sgr & \(19^{\rm h}10^{\rm m}\) & \(-21^\circ02'\) \\
			\(h1\) Sgr & \(19^{\rm h}37^{\rm m}\) & \(-20^\circ40'\) \\
			\hline
		\end{tabular}
	\end{table}
\begin{sol}
	...
	\begin{enumerate}[a.]
		...
		\item The stereographic projection is centered on Arcturus (\(\alpha\) Boo), whose coordinates are
		\[
		\alpha_0=14^{\rm h}16^{\rm m},
		\qquad
		\delta_0=+19^\circ03'.
		\]
		
		The coordinates of \(\gamma\) Sgr are
		\[
		\alpha_\gamma=18^{\rm h}07^{\rm m},
		\qquad
		\delta_\gamma=-30^\circ25'.
		\]
		
		The angular separation between the two stars is
		\[
		\cos\theta
		=
		\sin\delta_0\sin\delta_\gamma
		+
		\cos\delta_0\cos\delta_\gamma
		\cos(\alpha_\gamma-\alpha_0).
		\]
		...
		From the chart, the projected distance between Arcturus and \(\gamma\) Sgr is measured to be
		
		\[
		r=6.10\ {\rm cm}. \tag{This is a representative value}
		\]
		...
		The Milky Way is drawn as the projection of the galactic equator, i.e. the great circle whose pole is the North Galactic Pole. The Sagittarius Dwarf Galaxy has an angular thickness of approximately $	12^\circ.$
		
		The corresponding projected width is
		
		\[
		w
		=
		2R\tan\left(\frac{12^\circ}{2}\right)
		=
		2(4.02)\tan(6^\circ)
		=
		0.84\ {\rm cm}.
		...
		This width is used when sketching the dwarf galaxy on the chart.
		
		\item
		
		The apparent boundary of the Sagittarius Dwarf Galaxy passes through
		\[
		\gamma{\rm\ Sgr},
		\qquad
		\pi{\rm\ Sgr},
		...
		The coordinates of the relevant stars are
	\begin{gather*}
				\alpha_\pi=19^{\rm h}10^{\rm m},
		\qquad
		\delta_\pi=-21^\circ02',\\
				\alpha_{h1}=19^{\rm h}37^{\rm m},
		\qquad
		\delta_{h1}=-20^\circ40',\\
		\alpha_\gamma=18^{\rm h}07^{\rm m},
		\qquad
	...
	\end{gather*}
		...
		The angular distance between \(\pi\) Sgr and \(h1\) Sgr is
		
		\[
		\cos\theta_{\pi h1}
		=
		\sin\delta_\pi\sin\delta_{h1}
		+
		\cos\delta_\pi\cos\delta_{h1}
		\cos(\alpha_{h1}-\alpha_\pi),
		\]
		...
		The corresponding projected distance is
		
		\[
		r_{\pi h1}
		=
		2R\tan\left(\frac{\theta_{\pi h1}}{2}\right)
		=
		2(4.02)\tan\left(\frac{7.19^\circ}{2}\right)
		=
		0.505\ {\rm cm}.
		\]
		...
		\cos\theta_{\gamma h1}
		=
		\sin\delta_\gamma\sin\delta_{h1}
		+
		\cos\delta_\gamma\cos\delta_{h1}
		\cos(\alpha_{h1}-\alpha_\gamma),
		\]
		...
		r_{\gamma h1}
		=
		2R\tan\left(\frac{\theta_{\gamma h1}}{2}\right)
		=
		2(4.02)\tan\left(\frac{20.73^\circ}{2}\right)
		=
		1.47\ {\rm cm}.
		\]
		
		On the chart, a circle of radius \(0.505\) cm is drawn around \(\pi\) Sgr and a circle of radius \(1.47\) cm is drawn around \(\gamma\) Sgr. Their intersection determines the location of \(h1\) Sgr. The three stars
		\[
		\gamma{\rm\ Sgr},
		\qquad
		\pi{\rm\ Sgr},
		...
		define the visible boundary of the Sagittarius Dwarf Galaxy. An elongated spheroidal outline passing through these stars is then drawn.
		
		\item
		
		The Sagittarius Dwarf Galaxy is approximately spheroidal. After constructing its outline, the geometric center of the figure is determined.
		
		The center is marked on the chart with a ``+'' symbol and represents the location of the galaxy's nuclear star cluster.
		
		\item
		
		The globular cluster located at the center of the Sagittarius Dwarf Galaxy is
...
\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=\linewidth]{sgr_dwarf_prob}}
	\caption{Work on the Map yourself!}
	\label{sgr_dwarf_map}
\end{figure}
...
\begin{table}[H]
	...
	\begin{tabular}{|c|l|c|l|}
		\hline
		System & \multicolumn{1}{c|}{Coordinates} & Interval & Reference point \\ \hline \hline
		
		Equatorial &
		\begin{tabular}[c]{@{}l@{}}$\delta$: declination\\ $\alpha$: right ascension\end{tabular} &
		\begin{tabular}[c]{@{}c@{}}$-90^\circ \leq \delta \leq +90^\circ$\\ $0^h \leq \alpha \leq 24^h$\end{tabular} &
		\begin{tabular}[c]{@{}l@{}}Celestial equator\\ First point of Aries\end{tabular}
		\\ \hline
		
		Horizontal &
		\begin{tabular}[c]{@{}l@{}}$a$: altitude\\ $A$: azimuth\end{tabular} &
		\begin{tabular}[c]{@{}c@{}}$-90^\circ \leq a \leq +90^\circ$\\ $0^\circ \leq A \leq 360^\circ$\end{tabular} &
		\begin{tabular}[c]{@{}l@{}}Horizon\\ North point\end{tabular}
		\\ \hline
		
		\begin{tabular}[c]{@{}c@{}}Equatorial\\ Time\end{tabular} &
		\begin{tabular}[c]{@{}l@{}}$\delta$: declination\\ $H$: hour angle\end{tabular} &
		\begin{tabular}[c]{@{}c@{}}$-90^\circ \leq \delta \leq +90^\circ$\\ $0^h \leq H \leq 24^h$\end{tabular} &
		\begin{tabular}[c]{@{}l@{}}Celestial equator\\ Local meridian\end{tabular}
		\\ \hline
		
		Ecliptic &
		\begin{tabular}[c]{@{}l@{}}$\beta$: ecliptic latitude\\ $\lambda$: ecliptic longitude\end{tabular} &
		\begin{tabular}[c]{@{}c@{}}$-90^\circ \leq \beta \leq +90^\circ$\\ $0^\circ \leq \lambda \leq 360^\circ$\end{tabular} &
		\begin{tabular}[c]{@{}l@{}}Ecliptic plane\\ First point of Aries\end{tabular}
		\\ \hline
		
		Galactic &
		\begin{tabular}[c]{@{}l@{}}$B$: galactic latitude\\ $L$: galactic longitude\end{tabular} &
		\begin{tabular}[c]{@{}c@{}}$-90^\circ \leq B \leq +90^\circ$\\ $0^\circ \leq L \leq 360^\circ$\end{tabular} &
		\begin{tabular}[c]{@{}l@{}}Galactic plane\\ Galactic center\end{tabular}
		\\ \hline
		
	\end{tabular}
	\caption{Summary of Celestial Coordinate Systems}
\end{table}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{stereo_projection.pdf}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.6\linewidth]{Hor_1.png}}
...
\subsubsection{Round Maps}\label{stereo_projection}
...
\end{defi}
\subsubsection*{Altitude from Stereographic charts}

Consider a stereographic projection from the nadir onto a plane tangent to the celestial sphere at the zenith. Let $P$ be a point on the celestial sphere, $P'$ its stereographic projection on the chart, $R$ the radius of the chart, $r$ the distance from the chart center to $P'$, $z$ the zenith distance of $P$, and $a$ its altitude.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\linewidth]{chart_geometry.pdf}
	\caption{Stereographic Projection Geometry}
	\label{chartgeo_11}
\end{figure}

From the geometry of the stereographic projection (see figure \ref{chartgeo_11}), the following
relation holds:
\[
\tan\!\left(\frac{z}{2}\right) = \frac{r}{R}.
\]

Solving for the zenith distance, we obtain
\[
z = 2 \arctan\!\left(\frac{r}{R}\right).
\]

Since altitude and zenith distance satisfy $a + z = 90^\circ,$ the altitude of the point $P$ is therefore
\[\boxed{a = 90^\circ - 2 \arctan\!\left(\dfrac{r}{R}\right)}\]


\textbf{Steps to follow with printed charts to find} $\bm{\phi}$\textbf{ with Polaris}
\begin{enumerate}[Step 1]
	\itemsep0em
	\item Identify the zenith point (chart center) and the horizon circle on the stereographic chart.
	\item Measure the chart radius $R$, defined as the distance from the chart center (zenith) to the horizon circle.
	\item Measure the radial distance $r_P$ from the chart center (zenith) to the projected position of Polaris.
	\item Compute the zenith distance $z$ using the stereographic relation
	\[
	z = 2 \arctan\!\left(\frac{r_P}{R}\right).
	\]
	\item Convert zenith distance to altitude using
	\[
	a_P = 90^\circ - z.
	\]
	\item Since the altitude of Polaris approximates the observer’s latitude, take
	\[
	\phi \approx a_P.
	\]
\end{enumerate}
...
\clearpage
\subsection{Pole View Maps}\label{pole_view_maps}
...
\begin{figure}[H]
    ...
    \caption{View from North Celestial Pole \href{https://watcheroftheskies.net/constellations/northern.pdf}{HQ PDF}}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{View from South Celestial Pole \href{https://watcheroftheskies.net/constellations/southern.pdf}{HQ PDF}}
\end{figure}
...
The Earth orbits the Sun in a plane that we call the \emph{plane of the ecliptic}. The Earth's rotation axis is tilted by an angle of $\varepsilon = 23.5^\circ$ with respect to the direction perpendicular to the ecliptic plane. In other words, the ecliptic and celestial poles are separated on the celestial sphere by the same angle $\varepsilon$. The Earth's northern hemisphere is most tilted toward the Sun during the \emph{summer solstice} and most tilted away during the \emph{winter solstice}.\\
...
The period of Earth's precession is called the \emph{Platonic year}, which lasts $25{,}800$ years. The same kind of motion can be observed in a rapidly spinning \emph{top}, whose rotation axis is slightly tilted and slowly changes its direction in space. The precessional motion causes the north and south rotational poles
to trace out circles around the pole of the ecliptic (in $T_{pr}\approx 25800$ year). Currently the Earth’s north celestial pole points towards the constellation \emph{Ursa Minor}, near the \emph{North Star (Polaris)}, but this will not always be the case. By 2500, Polaris will be several degrees away from the true celestial pole.

\begin{table}[H]
	\centering
	\begin{tabular}{|c|c|c|c|c|c|}
		\hline
		\textbf{Year} & \textbf{Star} & \textbf{Star Designation} & \textbf{Constellation} & \textbf{Magnitude} & \textbf{Dist. From North} \\
		\hline
		-11700 & Vega & $\alpha$ Lyr & Lyra & 0.03 & 3.3$^\circ$ \\ 
		\hline
		-10000 & -- & $\tau$ Her & Hercules & 3.89 & 1.6$^\circ$ \\ 
		\hline
		-5500 & -- & $\iota$ Her & Hercules & 3.80 & 0.4$^\circ$ \\ 
		\hline
		-5000 & Edasich & $\iota$ Dra & Draco & 3.29 & 2.8$^\circ$ \\ 
		\hline
		-3000 & Thuban & $\alpha$ Dra & Draco & 3.65 & 0.1$^\circ$ \\ 
		\hline
		-1000 & Kochab & $\beta$ UMi & Ursa Minor & 2.08 & 4.2$^\circ$ \\ 
		\hline
		\rowcolor{gray!25}
		2000 & Polaris & $\alpha$ UMi & Ursa Minor & 2.02 & 0.3$^\circ$ \\ 
		\hline
		4000 & Errani & $\gamma$ Cep & Cepheus & 3.21 & 1.3$^\circ$ \\ 
		\hline
		6000 & Alfrik & $\beta$ Cep & Cepheus & 3.23 & 2.9$^\circ$ \\ 
		\hline
		7500 & Alderamin & $\alpha$ Cep & Cepheus & 2.45 & 1.5$^\circ$ \\ 
		\hline
		10000 & Deneb & $\alpha$ Cyg & Cygnus & 1.25 & 4.2$^\circ$ \\ 
		\hline
		11500 & -- & $\delta$ Cyg & Cygnus & 2.87 & 1.4$^\circ$ \\ 
		\hline
		14000 & Vega & $\alpha$ Lyr & Lyra & 0.03 & 3.3$^\circ$ \\ 
		\hline
	\end{tabular}
	\caption{Precession of the North Celestial Pole and its nearest bright stars over time.}
\end{table}
%https://www.coconinoastro.org/PDF/Precession.pdf 
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6 \linewidth]{presc1.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.3 \linewidth]{potd65a.pdf}
...
\begin{pro}
	\begin{enumerate}[a.]
		\itemsep0em 
		\item What was the right ascension of the ecliptic pole $\alpha'_{\rm E}$ 12,900 years ago? The vernal equinox is always defined as the intersection point of the celestial equator and the ecliptic, where the Sun is located during the spring equinox. \emph{Note:} 12,900 years corresponds to half of the Platonic year.
		
		\item The star Betelgeuse currently has coordinates $\alpha = 6^\mathrm{h}$, $\delta = 7^\circ$. What were its coordinates $\alpha'$, $\delta'$ 12,900 years ago? Neglect the star’s proper motion and nutation.
		
		\item Was Betelgeuse observable from Slovakia 12,900 years ago as a circumpolar, setting, or rising star? Assume the observer’s geographical latitude $\varphi = 49^\circ$.
	\end{enumerate}
\end{pro}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\linewidth]{prescession_geometry.pdf}
	\caption{Diagram of the precession of the celestial pole around the ecliptic pole.}
	\text{\prt{Pink} shows the situation 12,900 years ago and {\color{blue} blue} shows the situation today.}
\end{figure}
\begin{pro}
...
\subsection*{Finding new Pole Star [Using Star Chart]}
...
\begin{tabular}{|l|l|l|}
	\hline
	\textbf{Planet} & \textbf{North Polar Star}                                                                                        & \textbf{South Polar Star}                                                                                                         \\ \hline
	Mercury         & $\mathrm{o}$-Dra                                                                                                 & $\alpha$-Pic                                                                                                                      \\ \hline
	Venus           & $\eta^{1}$-Dor                                                                                                   & 42-Dra                                                                                                                            \\ \hline
	Mars            & \begin{tabular}[c]{@{}l@{}}Extension of one times the line \\ joining $\gamma$-Cyg and $\alpha$-Cyg\end{tabular} & $\kappa$-Vel                                                                                                                      \\ \hline
	Jupiter         & $\zeta$-Dra                                                                                                      & $\delta$-Dor                                                                                                                      \\ \hline
	Saturn          & \begin{tabular}[c]{@{}l@{}}Forms a right triangle with \\ $\alpha$-UMi and $\gamma$-Cep\end{tabular}             & $\delta$-Oct                                                                                                                      \\ \hline
	Uranus          & $\eta$-Oph                                                                                                       & \begin{tabular}[c]{@{}l@{}}15-Ori / Extension of one \\ times the line joining \\ $\gamma$-Ori and $\varepsilon$-Ori\end{tabular} \\ \hline
	Neptune         & \begin{tabular}[c]{@{}l@{}}Midpoint of the line joining \\ $\gamma$-Cyg and $\delta$-Cyg\end{tabular}            & $\gamma$-Vel                                                                                                                      \\ \hline
\end{tabular}
	\caption{Stars located near the north and south celestial poles of the planets.}
\end{table}
...
As you can see in Fig. , it is created by projecting the points of the sphere onto a cylindrical surface that touches the sphere, usually at the equator. The advantage is that it preserves directions and both declination circles and right ascension circles are displayed as straight lines (in the ``left-right" or ``up-down" directions), but the disadvantage is a large distortion in the area of ​​the poles. Looking at the projection method, you can easily imagine that the map cannot display the entire sphere and some part around the poles is omitted.

%https://watcheroftheskies.net/constellations/maps.html 

\subsubsection{Determining Equatorial Coordinates Using Mercator and Pole View Maps}

In the Mercator projection, points on the celestial sphere are mapped onto a cylindrical surface tangent at the equator. The horizontal coordinate corresponds to right ascension $\alpha$, while the vertical coordinate $y$ is related to declination $\delta$ through
\[
y = \mathcal{R} \ln \left( \tan \left( \frac{\pi}{4} + \frac{\delta}{2} \right) \right),
\]
where $\mathcal{R}$ is the scale parameter of the Mercator projection. It is determined directly from the map by measuring the total length of the equator. If $L_{\text{eq}}$ is the measured width of the map along the equator, then
\[
2\pi \mathcal{R} = L_{\text{eq}}, \qquad \mathcal{R} = \frac{L_{\text{eq}}}{2\pi}.
\]


In this projection, lines of constant right ascension and declination form an orthogonal grid. This allows the equatorial coordinates $(\alpha,\delta)$ of stars to be determined directly from measurements on the map. 

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{mercator_1.png}
\end{figure}

\paragraph{Determination of declination:}
In practice, only a finite range of declination is shown on the mercator map. The declination is obtained from the measured vertical coordinate using the inverse relation
\[
\delta = 2\left[\arctan\left(e^{y/\mathcal{R}}\right) - \frac{\pi}{4}\right],
\]
where $y$ is measured directly on the map (in the same units as $L_{\text{eq}}$). If working in degrees, the term $\pi/4$ is replaced by $45^\circ$.

\paragraph{Determination of right ascension:}
The right ascension is obtained from the horizontal coordinate. If $x$ denotes the measured horizontal coordinate and $x_0$ the position of the vernal equinox, then
\[
\alpha = 24^\mathrm{h} \cdot \frac{x - x_0}{L_{\text{eq}}},
\]
since the full equator corresponds to $24^\mathrm{h}$ and has total length $L_{\text{eq}}$.

\paragraph{Transfer to the stereographic map:}
Once the coordinates $(\alpha,\delta)$ are obtained, the stars are plotted onto the stereographic (pole view \ref{pole_view_maps}) map. In this projection, the center corresponds to the celestial pole ($\delta = \pm 90^\circ$). The right ascension determines the angular position around the center (measured from the direction $\alpha = 0^\mathrm{h}$).

\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\linewidth]{stereo_plot.pdf}
\end{figure}

The declination determines the radial distance from the center. In an exact stereographic projection, this relation is given by
\[
\tan\!\left(\frac{90^\circ - \delta}{2}\right) = \frac{r}{R},
\]
where $r$ is the distance from the center of the map and $R$ is the radius of the stereographic chart.

However, as stated in the problem, this relation is approximated as linear. Thus, in practice, the radial distance is taken to be proportional to $90^\circ - \delta$. In particular, $\delta = 0^\circ$ corresponds to the boundary of the map, while $\delta = \pm 90^\circ$ corresponds to the center.


\begin{defi}
	{\color{red}\textbf{Caution:}} The quantities $\mathcal{R}$ (Mercator scale parameter) and $R$ (radius of the stereographic chart) are independent and arise from different projections. The Mercator map is used to determine the coordinates $(\alpha,\delta)$, while the stereographic map is used to represent their geometric positions.
\end{defi}


\paragraph{Accuracy and deviations:}
The coordinates obtained from the map are subject to measurement uncertainties. If the smallest division of the ruler is $1\,\mathrm{mm}$, a reasonable estimate of the measurement uncertainty is $\sigma_x \approx \sigma_y \approx 0.5\,\mathrm{mm}$.

The uncertainty in declination is obtained by propagating the error through the relation
\[
\delta = 2\left[\arctan\left(e^{y/\mathcal{R}}\right) - \frac{\pi}{4}\right],
\]
where $\mathcal{R}$ is determined from the measured equator length $L_{\text{eq}} = 2\pi \mathcal{R}$. Substituting typical values of $y$ and $\mathcal{R}$ obtained from the map gives an uncertainty of order
\[
\sigma_\delta \sim 1^\circ,
\]
with the exact value depending on the position on the map due to the non-linear dependence.

The uncertainty in right ascension follows from
\[
\alpha = 24^\mathrm{h} \cdot \frac{x - x_0}{L_{\text{eq}}},
\]
so that
\[
\sigma_\alpha = 24^\mathrm{h} \cdot \frac{\sigma_x}{L_{\text{eq}}}.
\]
Substituting typical values yields
\[
\sigma_\alpha \sim 0.05^\mathrm{h} \approx 3^\mathrm{m}.
\]

In addition to measurement errors, further deviations arise from the approximations used in the stereographic projection, in particular the assumption of a linear relation between radial distance and declination.
\clearpage
...
More Coming....[Very Soon est January 2027]
...
\end{sol}
\item \textbf{Is it possible for a star to be circumpolar if it passes through the zenith?}
\begin{sol}
	Only Polaris satisfies this condition. Otherwise, no.
\end{sol}
\item \textbf{What is the range of latitudes at which the sun can be circumpolar in a day?}
\begin{sol}
	As the axis of the Earth’s rotation is titled at an angle 23.5 degrees from the ecliptic, the sun can be circumpolar at latitude $66.5^\circ − 90^\circ$ N and $66.5^\circ − 90^\circ$ S.
\end{sol}
...
\end{table}
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.8\linewidth]{zodiac_dec.pdf}
	\end{figure}
...
Observation of Planets in the night sky is pretty easy. The first thing you need to do is, distinguished between countless dots in the night sky and figure which dot is a star and which one is a Planet!
	...
	\begin{tabular}{|llll|}
		...
		Jupiter & $\bm{\Jupiter}$ & Moon    & $\leftmoon$ \\
		\hline
	\end{tabular}
	\endgroup 
\end{table}
\begin{itemize}
	...
	\end{itemize}
	\item Above (same LOS) the zodiac constellations, bright but not twinkling $\implies$ definitely a Planet!
	\item Reddish planet which often demonstrates prominent retrograde motion $\implies$ \textbf{Mars} $\Mars$.
	\item Yellowish and smaller than Jupiter $\implies$ probably \textbf{Saturn} $\Saturn$. Ring should be observed with sufficiently powerful telescope.
\end{itemize}
...
\end{sol}
	...
	\begin{figure}[H]
		\centering
		\fbox{\includegraphics[width=0.6\linewidth]{elong_planets.eps}}
	\end{figure}
...
\clearpage
\subsubsection{Practice Telescope Tasks}
\begin{enumerate}
...
\item \textbf{Polar Alignment and Telescope Calibration}

An equatorial telescope is equipped with a polarscope and setting circles that allow celestial objects to be located using their equatorial coordinates. Before the telescope can be used to determine the coordinates of an unknown object, it must first be aligned with the north celestial pole and calibrated using a reference star of known right ascension and declination.

\begin{enumerate}[(a)]
	
	\item Align the telescope with the north celestial pole using the polarscope. Present the aligned telescope to the examiner for verification.
	
	The alignment will be evaluated according to the following schedule:
	
	\begin{center}
		\renewcommand{\arraystretch}{1.6}
		\begin{tabular}{|c|c|c|}
			\hline
			\textbf{First Verification} &
			\textbf{Second Verification} &
			\textbf{Subsequent Verifications} \\
			\hline
			100\% of the score &
			80\% of the score &
			60\% of the score \\
	...
	The star chart in Figure~\ref{tm_5} shows the position of a reference star together with its equatorial coordinates. Use this star to calibrate the telescope's setting circles.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{tm_6.1.png}
		\caption{Telescope Map T6.1}
		\label{tm_5}
	\end{figure}
	
	\[
	\alpha = 1^{\mathrm h}\,1^{\mathrm m},
	\qquad
	\delta = -1.0^\circ.
	\]
	Center this star in the telescope and use its known coordinates to calibrate the telescope's right ascension and declination setting circles. After calibration, present the telescope to the examiner for verification.
	\item Figure~\ref{tm_6} shows the position of another star. Using the calibrated telescope, determine its equatorial coordinates and record your measured values.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\textwidth]{tm_6.2.png}
		\caption{Telescope Map T6.2}
		\label{tm_6}
	\end{figure}
	...
	\begin{center}
		...
		\begin{tabular}{|p{8cm}|p{8cm}|}
			\hline
			\textbf{Right Ascension} & \textbf{Declination} \\
			\hline
			\rule{0pt}{1.2cm} & \rule{0pt}{1.2cm} \\
			\hline
		\end{tabular}
	\end{center}
...
%https://www.saguaroastro.org/wp-content/sac-images/NoviceLimitingMagnitude/limit-mag.pdf 
%https://project-nightflight.net/limiting_mag.pdf 

A measure of a star's relative dimness is called it's magnitude. The brightest stars are near magnitude zero (a few are even negative.) Stars can be easily seen in suburban skies down to 4th magnitude, and at dark sites, down to greater than 6th magnitude with the naked-eye.\\
...
\subsubsection*{T6. Example Problem: Noah's Ark}

{ \footnotesize
On the following pages you will observe an image of \textbf{Noah's Ark}\footnote{I found no official names for the star group in google but it sure exists in the sky!}. Using images numbered 1 through 4, place Noah's Ark at the center of the telescope eyepiece such that stars \textbf{A} through \textbf{F} are all visible in the field. Then present the telescope to the station supervisor for verification.\\ 

\begin{pro}
	\textbf{Note:} You may ask the supervisor to recenter Noah's Ark, but in that case you will receive only half of the score for this section.
\end{pro}
\vspace{0.3cm}
Using the image \ref{noah_ark} that you see in the eyepiece, answer the following questions.

\begin{enumerate}[a.]
	\itemsep0em
	\item Arrange stars \textbf{A} through \textbf{E} in order of brightness, from brightest to faintest.
	\item The apparent magnitude of star \textbf{B} is \(6.8\) and the apparent magnitude of star \textbf{D} is \(7.8\).
	
	Estimate the apparent magnitudes of stars \textbf{C} and \textbf{E}.

	\item Determine the cardinal directions \textbf{North}, \textbf{South}, \textbf{East}, and \textbf{West} around the edge of the field of view.
	\item Among stars \textbf{A} through \textbf{F}, which star has the smallest right ascension, and which star has the largest angular distance from the south celestial pole?
	\item Assume the angular separation between stars \textbf{D} and \textbf{F} is \(100\) units. In this scale, estimate the following quantities.
\end{enumerate}}
\begin{sol}
	\footnotesize
	\textbf{Suggested Method.}
	
	The apparent magnitudes of stars \(B\) and \(D\) are known:
	...
	The magnitude difference is $\Delta m = 1.0.$ We visually compare the brightnesses of stars \(C\) and \(E\) with the reference stars \(B\) and \(D\). If a star appears halfway in brightness between \(B\) and \(D\), its magnitude is estimated as $	m = 7.3.$ More generally, if the star appears to lie at a fraction \(f\) of the way from \(B\) to \(D\), then
	...
	The values of \(f\) are estimated visually from the eyepiece image, yielding the apparent magnitudes of stars \(C\) and \(E\).
\end{sol}
\begin{figure}[H]
	\centering
	
	\fbox{\includegraphics[width=0.48\linewidth]{noah1.png}}
	\hfill
	\fbox{\includegraphics[width=0.48\linewidth]{noah2}}
	
	\vspace{0.5em}
	
	\fbox{\includegraphics[width=0.48\linewidth]{noah3.png}}
	\hfill
	\fbox{\includegraphics[width=0.48\linewidth]{noah4.png}}
	
	\caption{Finder charts for Noah's Ark.}
	\label{noah_ark}
\end{figure}

\begin{defi}
	This is a simulated task you perform in actual Telescope Station. If you don't have access to a telescope practice this problem using Stellarium.
	\tcblower
\begin{table}[H]
	\centering
	\begin{tabular}{|c|c|}
		\hline
		\textbf{Apparent Magnitude of Star E} &
		\textbf{Apparent Magnitude of Star C} \\
		\hline
		\rule{0pt}{0.9cm} & \rule{0pt}{0.9cm} \\
		\hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{|c|c|}
		\hline
		\textbf{Star with Smallest Right Ascension} &
		\textbf{Star with Largest Angular Distance from SCP} \\
		\hline
		\rule{0pt}{0.9cm} & \rule{0pt}{0.9cm} \\
		\hline
	\end{tabular}
...
\begin{table}[H]
	...
	\begin{tabular}{|c|c|c|}
		\hline
		\textbf{Field of View} &
		\textbf{Angular Separation of B and E} &
		\textbf{Angular Separation of A and C} \\
		\hline
		\rule{0pt}{0.9cm} &
		\rule{0pt}{0.9cm} &
		\rule{0pt}{0.9cm} \\
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.8\linewidth]{BDOAA_19.png}}
\end{figure}
\begin{enumerate}[a.]
...
\end{table}
%Zenith       & 13.28 & -18.00  & -18.00  &  88.78 \\
{ \footnotesize
...
\begin{enumerate}
 ...
 \begin{table}[H]
 	...
 	\begin{minipage}{0.45\textwidth}
 		...
 		\begin{tabular}{|c|c|c|}
 			\hline
 			\textbf{Code} & \textbf{Name} & \textbf{Bayer Name} \\
 			\hline
 			S1 & Caph & $\beta$ Cas \\
...
%\end{figure}
 ...
 \begin{table}[h]
 	...
 	\begin{minipage}{0.45\textwidth}
 		...
 		\begin{tabular}{|c|c|c|}
 			\hline
 			\textbf{Code} & \textbf{Name} & \textbf{M number} \\
 			\hline
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.6\linewidth]{IOAA16_OM.jpg}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.95\linewidth]{18_O2.png}}
    \caption{O2 Map}
    \label{A2018_O2}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.95\linewidth]{18_O3.png}}
    \caption{O3 Map}
    \label{A2018_O3}
\end{figure}
\clearpage
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.9\linewidth]{18_O4.png}}
    \caption{O4 Map}
    \label{A2018_O4}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.95\linewidth]{18_O5.png}}
    \caption{O5 Map}
    \label{A2018_O5}
\end{figure}
\end{enumerate}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.95\linewidth]{GeCAA_1.png}}
...
\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.76\linewidth]{ioaa22_3.png}}
...
\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.95\linewidth]{ioaa22_4.png}}
	\caption{IOAA 2022 NO9}
\end{figure}
\clearpage
\subsection{IOAA 2025 Grid-style}
\textit{Author: Labib Bin Azad, BDOAA 26 TST}\\

The figure \ref{ioaa25_gridstyle} shows a Mercator projection of the celestial sphere with constellation boundaries and the grid of a certain celestial co-ordinate system. It is known that 


\begin{enumerate}[(a)]
	\item Mark the North and South Celestial poles on the map and label them with ``N" and ``S" respectively. \hfill [1 mark]
	\item Identify the blue shaded constellations which are marked from ``a" to ``e". \hfill [5 marks]
	\item Draw the outlines and write the IAU constellation names for the constellations in red shaded region marked from ``A" to ``H". \hfill [8 x 1.5 = 12 marks]
	\item Identify the great circles marked from ``1" to ``5" by labelling them clearly on the map. \hfill [5 marks]
	\item Mark and label the two equinoxes on the map  with \textbf{VE} and \textbf{AE}. \hfill [2 marks]
	\item Mark the solstice visible in this map, and also identify which solstice it is, and properly label it with ``\textbf{S}''. \hfill[2 marks]
	\item A colure is an astronomical term for either of the two principal meridians on the celestial sphere. These two great circles intersect each other at right angles at the celestial poles, dividing the celestial sphere into four quadrants. Draw and label the two principal colures of the celestial sphere:
	\begin{itemize}
		\item The Equinoctial Colure, passing through the North and South Celestial Poles and the vernal and autumnal equinoxes.
		\item The Solstitial Colure, passing through the North and South Celestial Poles and the summer and winter solstices.
	\end{itemize}
	
	Hence, write down the right ascension values associated with each colure. \hfill[2 marks]
	
	\item Mark the following stars in the sky map with their names as labels, and estimate their Co-ordinates. \hfill[10 marks]
	
	\begin{table}[H]
		\centering
		\begin{tabular}{|c|c|c|c|}
			\hline
			Star names & IAU Abbreviatiopns &Right Ascension ($\alpha$) & Declination ($\delta$) \\ \hline 
			Mimosa & $\beta$ Cru & & \\ \hline
			Capella & $\alpha$ Aur & & \\ \hline
			Alphecca & $\alpha$ CrB & & \\ \hline
			Alnair & $\alpha$ Gru & & \\ \hline
			Canopus & $\alpha$ Car & & \\ \hline
		\end{tabular}
	\end{table}
	
	
	\item  Mark any 3 messier objects with (+) in the given sky map from the following list, and also write down their types in your answer scripts:
	\begin{center}
		M83, M29, M57, M101, M4, M74, M70.
	\end{center}
...
\begin{figure}[H]
	\vspace{1cm}
	\centering
	\fbox{\includegraphics[angle=90,width=0.55\textheight]{IOAA25_gridstyle.png}}
	\caption{IOAA 2025-like Grid Task}
	\label{ioaa25_gridstyle}
\end{figure}
...
\begin{enumerate}
    ...
    \item  International Olympiad on Astronomy and Astrophysics (IOAA) and subsidiaries (IOAAjr \& GeCAA) --  \url{https://ioaastrophysics.org/}
...
\item \textbf{My IOAA Notes} -- \url{https://rajit13.github.io/olympiad.html#notes}\\
I have several highly curated notes on Olympiad Astronomy worth checking! 
\end{enumerate}
